...
main = string(datetime('now','Format','M-d-y@HH.mm.ss'))+"code5g";
mkdir(main);
...
ncoress = 1:28;
tgpu = zeros(length(ncoress),1);
tcpu = zeros(length(ncoress),1);

%gpu
for k = 1:length(ncoress)
    ncores = ncoress(k);
    p = gcp('nocreate');
    if isempty(p)
        p = parpool(ncores);
    ...
    fprintf("ncores = %d.\n", ncores);
    fprintf("\nRunning GPU scan.\n");
    angleGPU = zeros(ntheta*nphi,1);
    tstartGPU = tic;
    parfor i = 1:ntheta*nphi
        ...
        angleGPU(i) = acos((vx0*yend10+vy0*yend11+vz0*yend12)/sqrt(vx0^2+vy0^2+vz0^2)/sqrt(yend10^2+yend11^2+yend12^2));
    end
    tendGPU = toc(tstartGPU);
    tgpu(k) = tendGPU;
    fprintf("It took %.20f hours.\n",tgpu(k)/3600);
    dif = max(angleGPU(:)) - min(angleGPU(:));
    assert(dif==0);
...
for k = 1:length(ncoress)
    ...
    fprintf("ncores = %d.\n", ncores);
    fprintf("\nRunning CPU scan.\n");
    angleCPU = zeros(ntheta*nphi,1);
    tstartCPU = tic;
    parfor i = 1:ntheta*nphi
        [~,y] = ode45(@(t,y)g(C4,C8,mion,matom,ax,ay,az,qx,qy,qz,OmegaRF,t,y), [0,tmax], y00, options);
        angleCPU(i) = acos((vx0*y(end,10)+vy0*y(end,11)+vz0*y(end,12))/sqrt(vx0^2+vy0^2+vz0^2)/sqrt(y(end,10)^2+y(end,11)^2+y(end,12)^2));
    end
    tendCPU = toc(tstartCPU);
    tcpu(k) = tendCPU;
    dif = max(angleCPU(:)) - min(angleCPU(:));
    assert(dif==0);
    fprintf("Angle is supposed to be 1.84052171452610080493 radians.\n");
