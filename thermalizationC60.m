...
mion          = 720 * 1822.88848628276014097;
matom         = 4.00260325413 * 1822.88848628276014097;
Tion          = 300;   % Kelvin
Tatom         = 4;    % Kelvin
...
Cn            = 1.383/2;
Cm            = -1;
De            = 100 * 3.16681156345647751469e-06;
Re            = -1;
ax            = -2.982e-4;
ay            = ax;
az            = -2*ax;
...
tmax          = 2.1491e+11; %100e-9 / 2.4188843265864e-17;
r0            = 100;
ntrajectories = 120;        % unitless
...
ncores        = 40;         % unitless
positions     = "random";  % "uniform" or "random"
collisiontype = "thermal"; % "head-on" or "thermal"
...
% thermalization
tion          = 1 * 2*pi / (1/2*OmegaRF*sqrt(ax+1/2*qx^2));
ncol          = 200000;
...
main = string(datetime('now','Format','M-d-y@HH.mm.ss'))+"thermalization";
mkdir(main);
...
fprintf("For thermalization, we will simulate each trajectory with %d collisions, one after the other.\n", ncol);



temperatures = ones(ntheta,nphi,ncol+1);
probabilities = zeros(ncol+1,1);

tends       = zeros(ntheta,nphi);
yend1s      = zeros(ntheta,nphi);
yend2s      = zeros(ntheta,nphi);
yend3s      = zeros(ntheta,nphi);
yend4s      = randn(ntheta,nphi)*sqrt(kB*Tion/Eh/mion);
yend5s      = randn(ntheta,nphi)*sqrt(kB*Tion/Eh/mion);
yend6s      = randn(ntheta,nphi)*sqrt(kB*Tion/Eh/mion);
...
for icol = 1:ncol
    ...
    fprintf("Starting collision %d/%d.\n",icol,ncol);
    ...
    valids = sign(temperatures(:,:,icol));
    tions = ones(ntheta,nphi) * tion + tion*rand(ntheta,nphi);
    r0s = ones(ntheta,nphi) * r0;
    sigma = sqrt(kB*Tatom/Eh/matom);
    sigmas = ones(ntheta,nphi) * sigma;
    ...
    t0s = tends;
    tmaxs = t0s + tmax + tmax*rand(size(t0s));
    y01s = yend1s;
    y02s = yend2s;
    y03s = yend3s;
    y04s = yend4s;
    ...
    validsp = cell(ncores,1);
    tionsp = cell(ncores,1);
    r0sp = cell(ncores,1);
    sigmasp = cell(ncores,1);
    ...
    for i = 1:ncores
        ...
        validsp{i} = g(valids(floor(ntrajectories/ncores*(i-1))+1 : floor(ntrajectories/ncores*i)));
        tionsp{i} = g(tions(floor(ntrajectories/ncores*(i-1))+1 : floor(ntrajectories/ncores*i)));
        r0sp{i} = g(r0s(floor(ntrajectories/ncores*(i-1))+1 : floor(ntrajectories/ncores*i)));
        sigmasp{i} = g(sigmas(floor(ntrajectories/ncores*(i-1))+1 : floor(ntrajectories/ncores*i)));
    ...
    parfor i = 1:ncores
        [tendsp{i}, yend1sp{i}, yend2sp{i}, yend3sp{i}, yend4sp{i}, yend5sp{i}, yend6sp{i}, ...
            ...
            anglep{i}, bouncesp{i}, lifetimep{i}, positionp{i}, transferp{i}, distp{i}, nstepsp{i}, KEp{i}, tempionp{i}] = ...
            ...
            y07sp{i}, y08sp{i}, y09sp{i}, y010sp{i}, y011sp{i}, y012sp{i}, rtolsp{i}, atolsp{i}, ...
            maxstepssp{i}, validsp{i}, tionsp{i}, r0sp{i}, sigmasp{i});
    end
    ...
    for i = 1:ncores
        ...
        tempion(floor(ntrajectories/ncores*(i-1))+1 : floor(ntrajectories/ncores*i)) = gather(tempionp{i});
    end

    temperatures(:,:,icol+1) = tempion;
    probabilities(icol+1) = length(bounces(bounces>=2))/length(bounces(:));
    fprintf("Traj left: %d/%d. Tmean: %.7fK. std(T): %.7fK. P(complex): %.4f.\n", ...
        length(tempion(tempion>0)), length(tempion(:)), mean(tempion(tempion>0))*3.1577502480398e5, ...
        std(tempion(tempion>0))*3.1577502480398e5 / sqrt(length(tempion(tempion>0))), probabilities(icol+1));
end
...
if extraplots == "yes"
    ...
    if longestlived == "yes"
        ...
        if processor == "GPU"
            ...
            [tlend,ylend1,ylend2,ylend3,ylend4,ylend5,ylend6,ylend7,ylend8,ylend9,ylend10,ylend11,ylend12,...
                anglel,bouncesl,lifetimel,positionl,transferl,distl,nstepsl,KEl,tempionl] = ...
                ...
                y07s(imax),y08s(imax),y09s(imax),y010s(imax),y011s(imax),y012s(imax),rtol,atol,maxsteps,+1,tion,r0,sigma);
    ...
    if custom == "yes"
        ...
        [tcend,ycend1,ycend2,ycend3,ycend4,ycend5,ycend6,ycend7,ycend8,ycend9,ycend10,ycend11,ycend12,...
            anglec,bouncesc,lifetimec,positionc,transferc,distc,nstepsc,KEc,tempionc] = ...
            ode45gpu(mion,matom,n,m,Cn,Cm,ax,ay,az,qx,qy,qz,OmegaRF,rbounce,...
            t0custom,tfcustom,y0c(1),y0c(2),y0c(3),y0c(4),y0c(5),y0c(6),y0c(7),y0c(8),y0c(9),y0c(10),y0c(11),y0c(12),rtol,atol,maxsteps,+1,tion,r0,sigma);
...
e = zeros(1,ncol);
for icol = 1:ncol
    tmatrix = temperatures(:,:,icol+1) * 3.1577502480398e5;
    tmatrix = tmatrix(tmatrix>0);
    y(icol) = mean(tmatrix(:));
    e(icol) = std(tmatrix(:))/sqrt(length(tmatrix(:)));
end
errorbar(x,y,e);
title([sprintf("Thermalization of $^{%d}$Ion in", floor(mion/1822.88848628276014097)), ...
    sprintf("%.1fK bath of $^{%d}$Atoms",floor(Tatom),floor(matom/1822.88848628276014097))]);
xlabel("Number of collisions, $i_\textrm{col}$");
ylabel([sprintf("Average over %d trajectories of", ntheta*nphi), "ion ``temperature'', $T_\textrm{ion}$ (K)","$mv_\textrm{rms}^2/2=3k_BT/2$"]);
saveas(gcf,main+"/thermalization.png");
hold off;
...
for icol = 1:ncol
    tmatrix = temperatures(:,:,icol+1) * 3.1577502480398e5;
    tmatrix = tmatrix(tmatrix>0);
    y(icol) = median(tmatrix(:));
end
plot(x,y);
title([sprintf("Thermalization of $^{%d}$Ion in", floor(mion/1822.88848628276014097)), ...
    sprintf("%.1fK bath of $^{%d}$Atoms",floor(Tatom),floor(matom/1822.88848628276014097))]);
xlabel("Number of collisions, $i_\textrm{col}$");
ylabel([sprintf("Median over %d trajectories of", ntheta*nphi), "ion ``temperature'', $T_\textrm{ion}$ (K)","$mv_\textrm{rms}^2/2=3k_BT/2$"]);
saveas(gcf,main+"/thermalizationMedian.png");
hold off;
...
title([sprintf("Thermalization of $^{%d}$Ion in", floor(mion/1822.88848628276014097)), ...
    sprintf("%.1fK bath of $^{%d}$Atoms",floor(Tatom),floor(matom/1822.88848628276014097))]);
xlabel("Number of collisions, $i_\textrm{col}$");
ylabel([sprintf("Mean over %d trajectories of", ntheta*nphi), "ion ``temperature'', $T_\textrm{ion}$ (K)","$mv_\textrm{rms}^2/2=3k_BT/2$"]);
saveas(gcf,main+"/thermalizationMean.png");
hold off;

figure; hold on;
x = 1:ncol;
y = probabilities(2:end);
plot(x,y);
title([sprintf("Probabilities of complex formation of $^{%d}$Ion in", floor(mion/1822.88848628276014097)), ...
    sprintf("%.1fK bath of $^{%d}$Atoms",floor(Tatom),floor(matom/1822.88848628276014097))]);
xlabel("Number of collisions, $i_\textrm{col}$");
ylabel(["Probability of complex formation", "at $i^\textrm{th}$ collision, $P(i)$"]);
saveas(gcf,main+"/probabilities.png");
hold off;
...
function [tend,yend1,yend2,yend3,yend4,yend5,yend6,yend7,yend8,yend9,yend10,yend11,yend12,...
    angle,bounces,lifetime,position,transfer,dist,nsteps,KE,tempion] = ...
    ode45gpu(mion,matom,n,m,Cn,Cm,ax,ay,az,qx,qy,qz,OmegaRF,rbounce,...
    t0,tf,y01,y02,y03,y04,y05,y06,y07,y08,y09,y010,y011,y012,rtol,atol,maxsteps,valid,tion,r0,sigma)
    ...
    while dist == -1
        ...
        while ~done
            ...
            while true
                trials = trials + 1;
                if trials >= maxsteps
                    fprintf("Invalid trajectory: trials >= maxsteps in first loop.\n");
                    tend = -1;
                    yend1 = -1;
                    yend2 = -1;
                    yend3 = -1;
        ...
        if (y21-y27)^2 + (y22-y28)^2 + (y23-y29)^2 < 10 * ((y01-y07)^2 + (y02-y08)^2 + (y03-y09)^2)
            fprintf("Invalid trajectory: final distance %.20e less than 3 times initial distance %.20e after collision.\n", ...
                sqrt((y21-y27)^2 + (y22-y28)^2 + (y23-y29)^2), sqrt((y01-y07)^2 + (y02-y08)^2 + (y03-y09)^2));
            tend = -1;
            yend1 = -1;
            yend2 = -1;
            yend3 = -1;
            ...
            t0 = tend;
            tf = tend + tion;
            y01 = yend1; y02 = yend2; y03 = yend3; y04 = yend4; y05 = yend5; y06 = yend6;
            y07 = yend7; y08 = yend8; y09 = yend9; y010 = yend10; y011 = yend11; y012 = yend12;
            f01 = y04; f02 = y05; f03 = y06;                       %ode start
            f07 = y010; f08 = y011; f09 = y012;
            r = sqrt((y01-y07)^2+(y02-y08)^2+(y03-y09)^2);
            dVdr = 0;
            ...
            while ~done
                ...
                while true
                    trials = trials + 1;
                    if trials >= maxsteps
                        fprintf("Invalid trajectory: trials >= maxsteps in second loop.\n");
                        tend = -1;
                        yend1 = -1;
                        yend2 = -1;
                        yend3 = -1;
                ...
                f11 = f71; f12 = f72; f13 = f73; f14 = f74; f15 = f75; f16 = f76;
                f17 = f77; f18 = f78; f19 = f79; f110 = f710; f111 = f711; f112 = f712;
                weightedsum = weightedsum + mion * (y24^2 + y25^2 + y26^2) / 3 * (t2 - t1);
            end
            tempion = weightedsum / tion;
        end
        t0 = t0original;
        y01 = y01original;
        y02 = y02original;
