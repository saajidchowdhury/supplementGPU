...
for icol = 1:ncol %testing purposes. these are placeholder values
    temperatures(:,:,icol+1) = (2*rand(ntheta,nphi)-1)*0.3*exp(icol+1) + exp(icol+1);
end
...
if extraplots == "yes"
    ...
    if longestlived == "yes"
        ...
        if processor == "GPU"
            ...
            [tlend,ylend1,ylend2,ylend3,ylend4,ylend5,ylend6,ylend7,ylend8,ylend9,ylend10,ylend11,ylend12,...
                ...
                y07s(imax),y08s(imax),y09s(imax),y010s(imax),y011s(imax),y012s(imax),rtol,atol,maxsteps,+1,tion);
    ...
    if custom == "yes"
        ...
        [tcend,ycend1,ycend2,ycend3,ycend4,ycend5,ycend6,ycend7,ycend8,ycend9,ycend10,ycend11,ycend12,...
            ...
            ode45gpu(mion,matom,n,m,Cn,Cm,ax,ay,az,qx,qy,qz,OmegaRF,rbounce,...
            t0custom,tfcustom,y0c(1),y0c(2),y0c(3),y0c(4),y0c(5),y0c(6),y0c(7),y0c(8),y0c(9),y0c(10),y0c(11),y0c(12),rtol,atol,maxsteps,+1,tion);
...
e = zeros(1,ncol);
for icol = 1:ncol
    tmatrix = temperatures(:,:,icol+1) * 3.1577502480398e5 * 1e6;
    tmatrix = tmatrix(tmatrix>0);
    y(icol) = mean(tmatrix(:));
    e(icol) = std(tmatrix(:))/sqrt(ntheta*nphi);
end
errorbar(x,y,e);
title([sprintf("Thermalization of $^{%d}$Ion in", floor(mion/1822.88848628276014097)), ...
    sprintf("%.1f$\\mu$K bath of $^{%d}$Atoms",floor(Tatom*1e6),floor(matom/1822.88848628276014097))]);
xlabel("Number of collisions, $i_\textrm{col}$");
ylabel([sprintf("Average over %d trajectories of", ntheta*nphi), "ion ``temperature'', $T_\textrm{ion}$ ($\mu$K)"]);
...
function [tend,yend1,yend2,yend3,yend4,yend5,yend6,yend7,yend8,yend9,yend10,yend11,yend12,...
    ...
    ode45gpu(mion,matom,n,m,Cn,Cm,ax,ay,az,qx,qy,qz,OmegaRF,rbounce,...
    t0,tf,y01,y02,y03,y04,y05,y06,y07,y08,y09,y010,y011,y012,rtol,atol,maxsteps,valid,tion)
    ...
    if valid == +1
        ...
        if (y21-y27)^2 + (y22-y28)^2 + (y23-y29)^2 < (y01-y07)^2 + (y02-y08)^2 + (y03-y09)^2
            tend = -1;
            yend1 = -1;
            yend2 = -1;
            yend3 = -1;
