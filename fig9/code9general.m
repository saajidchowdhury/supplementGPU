...
main = string(datetime('now','Format','M-d-y@HH.mm.ss'))+"code9general";
mkdir(main);
...
for i = 1:length(ns)
    ...
    x0s = gpuArray(r0*sin(thetas)'*cos(phis));
    y0s = gpuArray(r0*sin(thetas)'*sin(phis));
    z0s = gpuArray(r0*cos(thetas)'*ones(1,nphi));
    vx0s = gpuArray(-v0*sin(thetas)'*cos(phis));
    vy0s = gpuArray(-v0*sin(thetas)'*sin(phis));
    vz0s = gpuArray(-v0*cos(thetas)'*ones(1,nphi));
    ...
    % x0s = (r0*sin(thetas)'*cos(phis));
    % y0s = (r0*sin(thetas)'*sin(phis));
    % z0s = (r0*cos(thetas)'*ones(1,nphi));
    % vx0s = (-v0*sin(thetas)'*cos(phis));
    % vy0s = (-v0*sin(thetas)'*sin(phis));
    % vz0s = (-v0*cos(thetas)'*ones(1,nphi));
    ...
    angle = acos((vx0s.*yend10+vy0s.*yend11+vz0s.*yend12)./sqrt(vx0s.^2+vy0s.^2+vz0s.^2)./sqrt(yend10.^2+yend11.^2+yend12.^2));
    ...
    saveas(gcf,main+sprintf('/angle%d.png',n));
    hold off;
end
...
clear angle atols axs ays azs C4s C8s matoms mions OmegaRFs qxs qys qzs rtols t0s tmaxs vx0s vxion0s vy0s vyion0s vz0s vzion0s x0s xion0s y0s yend1 yend10 yend11 yend12 yend2 yend3 yend4 yend5 yend6 yend7 yend8 yend9 yion0s z0s zion0s
clear y;
save(main+"/fig9general.mat");
