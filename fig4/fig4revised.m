...
mainhere = string(datetime('now','Format','M-d-y@HH.mm.ss'))+"fig4revised";
mkdir(mainhere);
...
plot(1:ntheta*nphi, cumsum(tcpug(:))/3600);
plot(1:ntheta*nphi, cumsum(tolderversion(:))/3600);
plot(1:ntheta*nphi, cumsum(tversiongpu(:))/3600);
xlim([-50,2530]);
ylim([-0.25,11]);

legend(["\verb+ode45+, $f(t,y)$","\verb+ode45+, $g(t,y)$","older version","\verb+ode45gpu+"],"Location","Northwest");
print(gcf,'-vector','-dsvg',mainhere+"/fig4revised.svg");
hold off;